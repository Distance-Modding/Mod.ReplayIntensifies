...
namespace Distance.ReplayIntensifies.Patches
{
	...
	internal static class LevelSelectLeaderboardMenu__Update
	{
		...
		internal static void Postfix(LevelSelectLeaderboardMenu __instance)
		{
			...
			if (Mod.EnableSteamRivals.Value && G.Sys.MenuPanelManager_.IsTop(__instance.gameObject))
			{
				...
				int numRivals = Mod.Instance.CountSteamRivals(picked.Select((entry) => entry.steamID));
				...
				if (buttonEnabled && __instance.inputManager_.GetKeyUp(InputAction.MenuSpecial_3, -2))
				{
					...
					if (!removeMode)
					{
						numChanged = picked.Count((entry) => Mod.Instance.AddSteamRival(entry.steamID, entry.entry.Name_, false));
					}
					else
					{
						numChanged = picked.Count((entry) => Mod.Instance.RemoveSteamRival(entry.steamID, false));
					}

					if (numChanged > 0)
					{
						// Wait until the end to save, in-case we have multiple entries selected.
						Mod.Instance.SaveDictionary(Mod.SteamRivals);
